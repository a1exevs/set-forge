...
type Props = {
  ...
  onRegister: (input: { email: string; password: string; redirectTo?: string }) => Promise<void>;
};
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent<HTMLFormElement>): Promise<void> => {
      ...
      if (activeTab === 'register') {
        ...
        try {
          await onRegister({ email: email.trim(), password, redirectTo });
        } catch (err) {
    ...
    [activeTab, captcha, email, onLogin, onRegister, password, redirectTo, resetFieldErrors, showCaptcha],
  );
