...
export namespace UpdateWorkoutListRequest {
  export class ExerciseDto {
    @ApiProperty({ required: false, example: 'b7e2d1f4-...', description: 'Id существующего упражнения' })
    @IsOptional()
    @IsUUID()
    readonly id?: string;
    ...
    @ApiProperty({ required: false, example: 0, description: 'Выполненные подходы (для существующих упражнений)' })
    @IsOptional()
    @IsInt({ message: ErrorMessages.ru.MUST_BE_AN_INTEGER_NUMBER })
    @Min(0, { message: ErrorMessages.ru.NUMERIC_MUST_NOT_BE_LESS_THAN_N.format(0) })
